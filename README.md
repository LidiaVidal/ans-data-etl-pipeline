...
## Autor
**Nome:** [Lídia Vidal]
[cite_start]**Linguagem escolhida:** [Python] 
...
## [cite_start]1. Instruções de Execução

### Pré-requisitos
* [Liste aqui o que precisa estar instalado. Ex: Python 3.9+, bibliotecas específicas]
...
### [cite_start]2.1 Processamento de Arquivos: Memória vs. Incremental 
...
* **Heterogeneidade de Formatos (CSV/TXT/XLSX):**
    Utilizei uma função de leitura robusta (`carregar_arquivo_robusto`) capaz de detectar e decodificar automaticamente diferentes *encodings* (`latin1` vs `utf-8`) e separadores (`;` vs `,`). [cite_start]Essa abordagem foi necessária pois a ANS não mantém um padrão estrito entre trimestres, e arquivos históricos frequentemente apresentam formatos legados[cite: 36].

* **Padronização de Colunas:**
    Desenvolvi um dicionário de mapeamento (`MAPA_COLUNAS`) que unifica nomenclaturas divergentes encontradas nos arquivos (ex: colunas como `'vl_saldo_final'` e `'valor_despesa'` são normalizadas para `'ValorDespesas'`). [cite_start]Isso soluciona o problema de esquemas de colunas variados sem a necessidade de intervenção manual[cite: 37].
...
* **Valores Numéricos e Negativos:**
    ...
    * **Justificativa:** Para a análise de sinistralidade proposta, o foco é a magnitude da despesa (valor positivo). [cite_start]Essa normalização corrige inconsistências onde o mesmo tipo de evento contábil aparecia ora como positivo, ora como negativo nos dados brutos[cite: 48, 50].

* **Resiliência a Estruturas de Diretórios:**
    Em substituição a caminhos estáticos, implementei uma varredura recursiva (`os.walk`) para localizar os arquivos de despesas. [cite_start]Essa decisão técnica endereça diretamente o alerta sobre trimestres que possuem múltiplas subpastas ou estruturas de diretório imprevisíveis após a descompactação[cite: 31].